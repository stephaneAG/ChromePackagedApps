...
(function(theWindow, theDocument, theUndefined){
  ...
  */
  ...
  // - data-ntfrmwrk-console-data-port              each of these will be added after one another with "/" appended in between
  // - data-ntfrmwrk-console-data-baudrate
  // - data-ntfrmwrk-console-data-linefeed
  ...
  var _console_name_color_default = '#FD971F'; // sublimetext orange as the default console name color
  ...
  var _console_data_show_baudrate_default = false; // the baud rate used will be shown in the displayed data ( by default after the name of the console )
  var _console_data_show_linefeed_default = false; // the line feed used will be shown in the displayed data ( by default after the name of the console )
  ...
  var _console_recv_prefixcolor_default = '#AD779E'; // sublimetext purple as the default color for the RECV prefix
  var _console_recv_prefixcssclass_default = 'console_recv_prefix_default'; // the default CSS class for the RECV prefix
  var _console_snd_prefix_default = '[SND] > '; // the default " SND " prefix
  var _console_snd_prefix_only_default = false; // by default, doesn't prevent recv prefixes to be displayed
  var _console_snd_prefixcolor_default = '#F92672'; // sublimetext red as the default color for the SND prefix
  var _console_snd_prefixcssclass_default = 'console_snd_prefix_default'; // the default CSS class for the RECV prefix

  var _console_suffixed_default = false; // the " RECV " & " SND " messages have a defaults suffix used to easily indentify them
  var _console_recv_suffix_default = '[RECV] > '; // the default " RECV " suffix
  var _console_recv_suffix_only_default = false; // by default, doesn't prevent snd suffixes to be displayed
  var _console_recv_suffixcolor_default = '#AD779E'; // sublimetext purple as the default color for the RECV suffix
  var _console_recv_suffixcssclass_default = 'console_recv_suffix_default'; // the default CSS class for the RECV suffix
  ...
  var _console_snd_suffixcolor_default = '#F92672'; // sublimetext red as the default color for the SND suffix
  var _console_snd_suffixcssclass_default = 'console_snd_suffix_default'; // the default CSS class for the RECV suffix  
  ...
  var _console_config_default = {
    ...
    recv_prefix_color:     _console_recv_prefixcolor_default,
    recv_prefix_cssclass:  _console_recv_prefixcssclass_default,
    snd_prefix:            _console_snd_prefix_default,
    snd_prefix_only:       _console_snd_prefix_only_default,
    snd_prefix_color:      _console_snd_prefixcolor_default,
    snd_prefix_cssclass:   _console_snd_prefixcssclass_default,
    ...
    recv_suffix_color:     _console_recv_suffixcolor_default,
    recv_suffix_cssclass:  _console_recv_suffixcssclass_default,
    snd_suffix:            _console_snd_suffix_default,
    snd_suffix_only:       _console_snd_suffix_only_default,
    snd_suffix_color:      _console_snd_suffixcolor_default,
    snd_suffix_cssclass:   _console_snd_suffixcssclass_default
  };
  ...
  var consoleReceive = function(serialConsole, messageStr){
    ...
    if( console_type == 'recvsnd' || console_type == 'recv' ) _appendLine(serialConsole, messageStr);
  ...
  var consoleWrite = function(serialConsole, messageStr){
    var console_type = serialConsole.getAttribute('data-ntfrmwrk-console-terminal-output');
    if( console_type == 'recvsnd' || console_type == 'snd' ) _appendLine(serialConsole, messageStr);
  ...
  var _appendLine = function(serialConsole, messageStr){
    ...
    if( _countLines(serialConsole) < console_maxlines ){ // there's enough space left in the configured lines buffer for us to write one more line to it
      ...
      if( console_direction == 'toptobottom' ) _appendToBottom(serialConsole, messageStr); // append to the bottom ( most commonly found / terminals default behavior )
      else _appendToTop(serialConsole, messageStr); // append to the top ( may be classy-sweet with to different consoles, one in each direction :p )
      ...
      if( console_direction == 'toptobottom' ){ // remove to top-most line & append to bottom
        ...
        _appendToBottom(serialConsole, messageStr); // append to the bottom of the console
      } 
        ...
        _appendToTop(serialConsole, messageStr); // append to the top of the console
  ...
  var _appendToTop = function(serialConsole, messageStr){
    ...
    */
    ...
    _restoreScrollTop // restore the previous scroll position
  };


  // - appendToBottom() - append a line above the current text present in the serial console
  var _appendToBottom = function(serialConsole, messageStr){
    ...
    */
    ...
    var formattedMessage = _formatMessage(serialConsole, messageStr);
  ...
  var _configure = function(serialConsole, configObject){
    ...
    var maxlines =            configObject.maxlines ? configObject.maxlines : _console_maxlines_default,
        ...
        data_color =           configObject.data_color ? configObject.data_color : _console_datacolor_default,
        data_cssclass =        configObject.data_cssclass ? configObject.data_cssclass : _console_datacssclass_default,
    ...
    if( _consoleGetAttribute(serialConsole, 'prefixed') == "true" ){ //console.log("[ SerialConsole ] Console1 supports prefixes !"); // if prefixes are activated
      if( _consoleGetAttribute(serialConsole, 'terminal-output') == "recvsnd" ){  // if the terminal output supports recv || snd prefixes
        ...
        if( _consoleGetAttribute(serialConsole, 'recv-prefix-only') == "false" && _consoleGetAttribute(serialConsole, 'snd-prefix-only') == "false" ){ // the console has no prefixes disabled
          ...
          console.log("[ SerialConsole ] Console1 allows both RECV & SND prefixes !");
          ...
          console.log("[ SerialConsole ] Console1 allows RECV prefixes only !");
          ...
          _consoleSetAttribute(serialConsole, 'snd-current-prefix', ''); // reset it just in case
          ...
          console.log("[ SerialConsole ] Console1 allows SND prefixes only !");
          ...
          _consoleSetAttribute(serialConsole, 'recv-current-prefix', ''); // reset it just in case

        } else {
          console.log("[ SerialConsole ] Console1 block all prefixes ??????? !");
        }
  ...
  var _formatMessage = function(serialConsole, messageStr){
    //console.log('_formatMessage:: ' + 'Prefix currently set: ' + _consoleGetAttribute(serialConsole, 'recv-current-prefix') );
    var fake_output_type = 'recv'; // fake the output type by now ( will be passed like: from "consoleReceive()"||"consoleWrite()" --> appendLine --> "appendToTop()"||"appendToBottom()" to here ;D )
    ...
    if( _consoleGetAttribute(serialConsole, 'prefixed') == "true" ){  // if prefixes are activated
      ...
      //if( fake_output_type == 'recv' && _consoleGetAttribute(serialConsole, 'snd-prefix-only') == "true" ) // do NOT append RECV prefix to the message
  ...
  var _formatMessage = function(serialConsole, messageStr){
    ...
    if( _consoleGetAttribute(serialConsole, 'suffixed') == "true" ){  // if prefixes are activated
      console.log("[ SerialConsole ] Formatting message with suffix..");
      formattedMessageStr +=_consoleGetAttribute(serialConsole, 'snd-current-prefix');
    }
    if( _consoleGetAttribute(serialConsole, 'prefixed') == "true" ){  // if prefixes are activated
      console.log("[ SerialConsole ] Formatting message with prefix..");
      formattedMessageStr = _consoleGetAttribute(serialConsole, 'recv-current-prefix') + formattedMessageStr;
      console.log('_formatMessage:: ' + 'Formatted Message Str: ' + formattedMessageStr );
      return formattedMessageStr; // return message with maybe suffix and prefix
    }
    return formattedMessageStr; // return message with maybe suffix and no prefix
  };
  */ 
