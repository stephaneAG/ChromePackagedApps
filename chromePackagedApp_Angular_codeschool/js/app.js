...
(function(){ // 'closure' or 'self executing anonymous functions'
  ...
  // app.controller('StoreController', function(){ // not using any services ( used with dependency injection ( .. ) )
  app.controller('StoreController', [ '$http', function($http){
    //this.product = gem; // set a property of our controller
    this.products = gems; // same as above but this time we have an array of products .. wich are still hardcoded & not fetched from an API :|
    // an example 'd be some data fetched from "http://api.example.com/products.json"
    //this.products = ???;
    
    // R: the code below can be used ONLY when NOT working with "file:///..."
    /*
    var store = this; // "DA ULTIMATE ANSWER ;D"
    store.products = []; // prevent any errors on page load ( aka, while the JSON data is being fetched )
    
    $http.get('/products.json').success(function(data){ // success is our Promises ;)
      store.products = data;
    });
    */
  //});
